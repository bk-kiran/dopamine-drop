  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      const syncRes = await fetch('/api/canvas/sync', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ isInitialSync: true }),
      })
      ...
      const coursesCount = syncData.courses ?? 0
      const assignments = syncData.synced ?? 0
      setSyncSummary(`Synced ${coursesCount} courses and ${assignments} assignments`)

      if (syncData.syncedCourses && syncData.syncedCourses.length > 0) {
        setSyncedCourses(syncData.syncedCourses)
        // Brief pause so user sees the "All done!" step before course selection
                ...
                <>
                  <div className="flex items-center justify-between p-5 border-b border-white/10">
                    ...
                    <button onClick={handleClose} className="p-1.5 hover:bg-white/10 rounded-lg transition-colors">
                      <X className="w-5 h-5 text-(--text-muted,#9ca3af)" />
                    </button>
                  </div>

                  <div className="p-5 space-y-4">
                    <div>
                      <label className="block text-sm font-medium text-(--text-muted,#9ca3af) mb-1.5">
                        Canvas API Token
                      </label>
                      <input
                        type="password"
                      ...
                      />
                      <p className="text-xs text-(--text-muted,#9ca3af) mt-2">
                    ...
                    <button
                      ...
                      disabled={!token.trim()}
                      className="w-full py-2.5 bg-linear-to-r from-purple-600 to-violet-600 hover:from-purple-500 hover:to-violet-500 disabled:from-gray-600 disabled:to-gray-600 disabled:cursor-not-allowed text-white rounded-xl font-semibold text-sm transition-all flex items-center justify-center gap-2"
                    >
                    ...
                    <button
                      onClick={handleClose}
                      className="w-full py-2.5 bg-white/5 hover:bg-white/10 text-(--text-muted,#9ca3af) hover:text-white rounded-xl text-sm font-medium transition-colors"
                    >
                      Maybe Later
                    </button>
                  </div>
